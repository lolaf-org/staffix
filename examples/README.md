...
setup, optional persistence and profiling. They are demonstrations rather than starting points, so read `quickstart`
first.
...
`has no incoming FixMessageDecoder` once per session on the way up, because this application maps none, which is
expected here, where session-level messages are the whole subject, and a sign of a missing decoder anywhere else.
