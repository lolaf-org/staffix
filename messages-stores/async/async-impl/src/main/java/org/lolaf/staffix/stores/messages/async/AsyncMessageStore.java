...
class AsyncMessageStore extends Startable.SimpleStartable<FixMessagesStore.FixSessionMessagesStore> implements FixMessagesStore.FixSessionMessagesStore {
    ...
    private void flushMessagesIfNeeded(Deadline flushDeadline) {
        if (!flushDeadline.isImmediate() && !messagesQueue.isEmpty()) {
            Duration deadlineDuration = flushDeadline.getRemainingTime();
            log.info("Waiting up to {} to flush {} messages for FIX session {}", deadlineDuration, messagesQueue.getSize(), fixSessionId);
            if (flushDeadline.waitAsLongAs(() -> !messagesQueue.isEmpty())) {
                log.info("All messages flushed for FIX session {}", fixSessionId);
            } else {
                log.warn("Unable to flush all messages for FIX session {} within {}, remaining {}", fixSessionId, deadlineDuration, messagesQueue.getSize());
            }
        }
    }
    ...
    public void find(long startSequenceNumber, long stopSequenceNumber, StoreMessagesConsumer consumer) {
        // Writes are applied to the wrapped store asynchronously, so wait for the queue to drain before reading to
        // avoid a resend missing messages that were stored just before this call.
        waitForQueueToDrainBeforeFindIfNeeded();
        wrappedStore.find(startSequenceNumber, stopSequenceNumber, consumer);
    }

    private void waitForQueueToDrainBeforeFindIfNeeded() {
        Duration timeout = asyncMessagesStoreSettings.getFindWaitForEmptyQueueTimeout();
        if (timeout == null || messagesQueue.isEmpty()) {
            return;
        }
        if (!Deadline.of(timeout).waitAsLongAs(() -> !messagesQueue.isEmpty())) {
            log.warn("Async messages queue for FIX session {} still not empty after waiting {} before find, {} messages pending",
                    fixSessionId, timeout, messagesQueue.getSize());
        }
    }
}