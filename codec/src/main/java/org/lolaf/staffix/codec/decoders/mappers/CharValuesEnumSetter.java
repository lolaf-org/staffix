...
class CharValuesEnumSetter<T extends FixField.CharValuesEnum, M> extends SetterBase {
    ...
    CharValuesEnumSetter(FixField field, Consumer<T> consumer, T resetValue, boolean fieldAutoResetEnabled) {
    ...
    CharValuesEnumSetter(FixField field, Consumer<M> consumer, Function<T, M> mapper, M resetValue, boolean fieldAutoResetEnabled) {
        super(field, fieldAutoResetEnabled);
    ...
    public void accept(SerDe.DeserializationContext ctx) {
        char providedValue = CharSerde.deserialize(ctx);
        FixField field = getField();
        for (Enum<?> candidate : field.getValues()) {
            FixField.CharValuesEnum enumValue = (FixField.CharValuesEnum) candidate;
            if (enumValue.code() == providedValue) {
