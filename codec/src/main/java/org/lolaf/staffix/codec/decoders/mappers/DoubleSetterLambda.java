...
class DoubleSetterLambda extends SetterBase {

    private final ToDoubleFunction<SerDe.DeserializationContext> deserializer;
    private final DoubleConsumer consumer;
    private final double resetValue;

    DoubleSetterLambda(FixField field, ToDoubleFunction<SerDe.DeserializationContext> deserializer, DoubleConsumer consumer, double resetValue, boolean fieldAutoResetEnabled) {
    ...
    public void accept(SerDe.DeserializationContext ctx) {
        consumer.accept(deserializer.applyAsDouble(ctx));
    }
