...
class IntValuesEnumSetter<T extends FixField.IntValuesEnum, M> extends SetterBase {
    ...
    IntValuesEnumSetter(FixField field, Consumer<T> consumer, T resetValue, boolean fieldAutoResetEnabled) {
    ...
    IntValuesEnumSetter(FixField field, Consumer<M> consumer, Function<T, M> mapper, M resetValue, boolean fieldAutoResetEnabled) {
        super(field, fieldAutoResetEnabled);
    ...
    public void accept(SerDe.DeserializationContext ctx) {
        int providedValue = IntSerde.deserialize(ctx);
        FixField field = getField();
        for (Enum<?> candidate : field.getValues()) {
            FixField.IntValuesEnum enumValue = (FixField.IntValuesEnum) candidate;
