...
class IntSetterVarHandle extends VarHandleSetterBase {

    private final ToIntFunction<SerDe.DeserializationContext> deserializer;
    private final int resetValue;

    IntSetterVarHandle(FixField field, ToIntFunction<SerDe.DeserializationContext> deserializer, VarHandle varHandle, Supplier<Object> varHandleTargetSupplier, int resetValue, boolean fieldAutoResetEnabled) {
    ...
    public void accept(SerDe.DeserializationContext ctx) {
        varHandle.set(varHandleTargetSupplier.get(), deserializer.applyAsInt(ctx));
    }
