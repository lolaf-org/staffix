...
| `threadLocalUuid(msb, lsb)` / `threadLocalUuid(MsbLsb)` | a UUID you already decoded | **nothing** | not measured |
...
its `rand_b` bits coming from a plain `ThreadLocalRandom`, which is the practical reason to prefer it on the message
...
`UUIDsGenerator.instance(id)` gives a generator dedicated to a subsystem. It is backed by `UUIDv7.instance(id)`, on the
...
| `CACHED` | rejected with an `IllegalStateException`, since a cache of unique identifiers would only grow |
